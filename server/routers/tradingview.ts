...
export const tradingViewRouter = router({
  ...
  receiveAlert: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const { symbol, exchange, action, price, indicators, message, timestamp } = input;
    ...
    .query(async ({ ctx }) => {
      ...
      return {
        ...
        instructions: {
          ...
          step3: "In the alert message tab, select 'Webhook URL'",
          step4: `Paste this URL: ${baseUrl}/api/trpc/tradingView.receiveAlert`,
          step5: "Set the message template to the JSON format shown in documentation",
        },
      };
    }),
