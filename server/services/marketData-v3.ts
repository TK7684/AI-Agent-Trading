...
interface CachedTickerData extends TickerData {
  lastUpdate: string;
}

interface CachedTrendingCoins {
  coins: any[];
  lastUpdate: string;
}

interface CachedGainersLosers {
  gainers: any[];
  losers: any[];
  lastUpdate: string;
}
...
class FreeMarketDataService {
  ...
  async getPrice(symbol: string): Promise<PriceData | null> {
    try {
      // Check cache first
      const cached = await cacheService.getPrice(symbol) as CachedPriceData | null;
      if (cached) {
  ...
  async getOHLCV(symbol: string, interval: string = '15m', limit: number = 100): Promise<OHLCVData[]> {
    try {
      // Check cache first
      const cached = await cacheService.getOHLCV(symbol, interval) as CachedOHLCVData | null;
      if (cached) {
      ...
      // Cache the result
      await cacheService.setOHLCV(symbol, interval, {
        symbol,
        interval,
        data,
  ...
  async getTicker24h(symbol: string): Promise<TickerData | null> {
    try {
      // Check cache first
      const cached = await cacheService.getTicker(symbol) as CachedTickerData | null;
      if (cached) {
      ...
      // Cache the result
      await cacheService.setTicker(symbol, {
        ...tickerData,
        lastUpdate: new Date().toISOString(),
      });
  ...
  async getTopCoins(limit: number = 50, currency: string = 'usd'): Promise<any[]> {
    try {
      // Check cache first
      const cached = await cacheService.getTrendingCoins() as CachedTrendingCoins | null;
      if (cached && cached.coins.length >= limit) {
      ...
      // Cache the result
      await cacheService.setTrendingCoins({
        coins,
        lastUpdate: new Date().toISOString(),
      });
  ...
  async getGainersLosers(top: number = 50): Promise<{ gainers: any[], losers: any[] }> {
    try {
      // Check cache first
      const cached = await cacheService.getGainersLosers() as CachedGainersLosers | null;
      if (cached) {
      ...
      // Cache the result
      await cacheService.setGainersLosers({
        ...result,
        lastUpdate: new Date().toISOString(),
      });
   ...
   */
  async getBatchPrices(symbols: string[]): Promise<Array<{ symbol: string; data: PriceData | null; error: any }>> {
